...
추가로 **Spring Security 기반 세션 로그인/로그아웃 기능**을 구현해  
서버 측 인증 및 세션 관리 흐름을 구성했습니다.  
(현재는 로그인/로그아웃 및 세션 확인까지만 제공)
...
- 인증 성공 시 HTTP Session을 생성합니다.
- 로그아웃 시 세션을 invalidate 처리합니다.
- 현재는 **세션 생성/해제 및 로그인 상태 확인까지만 구현**되어 있으며,
  API 접근 제어는 아직 적용하지 않았습니다.
